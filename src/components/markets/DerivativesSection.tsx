...
import { lakhs, latestOptionChains, longShare, net, participantOi, shortDate, type OptionChainEod, type ParticipantOi } from "@/lib/market-data";
...
function ChainView({ chains }: { chains: OptionChainEod[] }) {
  ...
  const chain = nearest.find((c) => c.symbol === symbol) ?? nearest[0];
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      <Card className="min-w-0 p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {nearest.map((c) => (
            <button key={c.symbol} type="button" onClick={() => setSymbol(c.symbol)} aria-pressed={c.symbol === chain.symbol}
...
/** F&O positioning: participant-wise open interest and the day's closing option chains. */
export default function DerivativesSection() {
  ...
  return (
    <section id="derivatives" aria-labelledby="derivatives-heading" className="scroll-mt-28 space-y-4">
      ...
      {chains.isLoading ? <Skeleton className="h-64 w-full" /> : <ChainView chains={chains.data ?? []} />}
    </section>
  );
}
