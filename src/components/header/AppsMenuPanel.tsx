  ...
  >
    <div className="container mx-auto grid gap-4 px-4 py-6 lg:grid-cols-[1.5fr_1fr_1fr]">
      ...
      <div className="relative flex gap-5 overflow-hidden rounded-2xl bg-hero p-5 text-primary-foreground">
        ...
        <Thumb src={MONEY_HERO.src} className="relative w-24 -rotate-3" />
        <div className="relative min-w-0">
          <span className="inline-flex rounded-full bg-secondary px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-secondary-foreground">New app</span>
          ...
          <ul className="mt-2 space-y-1 text-xs text-primary-foreground/85">
            <li>
              <Link to="/apps#tradetron" onClick={onClose} className="inline-flex items-center gap-1.5 hover:text-secondary">
                <Bot className="h-3.5 w-3.5 text-secondary" aria-hidden /> Tradetron no-code algo trading
              </Link>
            </li>
            <li>
              <Link to="/apps#margin-pledge" onClick={onClose} className="inline-flex items-center gap-1.5 hover:text-secondary">
                <Zap className="h-3.5 w-3.5 text-secondary" aria-hidden /> Instant margin pledge
              </Link>
            </li>
          </ul>
      ...
      >
        ...
        <span className="p-4">
          <span className="block font-semibold text-foreground group-hover:text-secondary">MoneyMaker Desktop</span>
          <span className="mt-0.5 block text-xs text-muted-foreground">The Windows terminal. Watch the 40-second tour.</span>
        </span>
      </Link>

      {/* Parasram Trade */}
      <div className="flex gap-4 rounded-2xl border border-border bg-background p-4">
        <Thumb src={TRADE_HERO.src} className="w-16" />
        <div className="min-w-0">
          <Link to="/apps#trade" onClick={onClose} className="font-semibold text-foreground hover:text-secondary">
            {trade.name}
          </Link>
          <p className="mt-0.5 text-xs text-muted-foreground">Symphony XTS app for Android and iPhone.</p>
      ...
      <div className="lg:col-span-3">
        ...
        >
          All apps, QR codes and downloads <ArrowRight className="h-4 w-4" aria-hidden />
        </Link>
      </div>
    </div>
  </motion.div>
