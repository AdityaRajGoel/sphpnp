...
const AppQrCodes = ({ app, className = "" }: Props) => {
  const codes = [
    { platform: "Android", src: APP_QR[app.id].android, href: app.playHref },
    { platform: "iPhone", src: APP_QR[app.id].ios, href: app.iosHref },
  ];
  return (
    <div className={`hidden md:flex gap-3 ${className}`}>
      {codes.map((c) => (
        <a
          ...
          aria-label={`Scan to get ${app.name} on ${c.platform}`}
        ...
        >
          <span className="rounded-xl bg-white p-1.5 shadow-md ring-1 ring-black/5 transition-transform duration-fast ease-out group-hover:scale-105">
            <img src={c.src} alt="" width={84} height={84} loading="lazy" decoding="async" className="h-[84px] w-[84px]" />
          </span>
